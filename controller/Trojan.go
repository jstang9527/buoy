	...
	for _, value := range getService {
		...
		if keyInfo != "" && strings.Contains(value.Name, keyInfo) || keyInfo == "" { // 2.关键词匹配
			if total >= skip { // 跳过多少条? 如果现在总量大于跳过总量
				assetID, assetName, isConn := utils.TrojanInfoHandler(value)
				outList = append(outList, &dto.TrojanItemOutput{AssetID: assetID, AssetName: assetName, IsConn: int8(isConn)})
			}
			total++
		}
	}
