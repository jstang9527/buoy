...
class Pocsuite:
    ...
    第三方库内置多线程
    仅支持对同一服务的多种漏洞进行扫描
    比如 ftp://127.0.0.1 与 http://127.0.0.1 是不同服务
    不支持对同一主机的多种服务进行同时扫描
    也鉴于RPC通信不宜进行太长的扫描时长
    '''
    def __init__(self, target: str, plugins: list, asset_id: str = None, command: str = None):
        self.target = target
...
if __name__ == "__main__":
    ...
    p = Pocsuite(targs, plugin_list, 'jstang9527')  # target: str, pocs: list, asset_id: str = None, command: str = None
    print(p.Verify(True))
